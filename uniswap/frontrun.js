...
async function preparedAttack(
    ...
    log_str =
        (input_token_info.balance / 10 ** input_token_info.decimals).toFixed(
            5
        ) +
        "\t" +
        input_token_info.symbol;
    ...
    const OUT_TOKEN_ABI_REQ =
        NETWORK == "mainnet"
            ? "https://api.etherscan.io/api?module=contract&action=getabi&address=" +
              out_token_address +
              "&apikey=33Y681VVRYXX7J2XCRX3CYYUSWPQ6EPQ9K"
    ...
    //check pool info
    if (
        (await getPoolInfo(WETH_TOKEN_ADDRESS, out_token_address, level)) ==
        false
    )
        return false;
