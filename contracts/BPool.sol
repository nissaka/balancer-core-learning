...
// https://balancer.gitbook.io/balancer/core-concepts/protocol/pool-lifecycle#pool-lifecycle
// https://balancer.gitbook.io/balancer/api/api

contract BPool is BBronze, BToken, BMath {
    // 类似于C语言，Solidity 也有结构体(struct)类型，用于表示复合型数据。结构体是引用类型。
    struct Record {
        bool bound; // is token bound to pool 限制
        uint256 index; // private
        uint256 denorm; // denormalized weight 非合规化的权重
        // 在正常的浮点数中， 有效数部分开头的不是0，0被移动到指数部分去了，比如0.0123会被表示为1.23*2-2。
        // Denormal numbers就是指数部分非常小的数字。这些数字的有效数部分首位用0来表示。
        uint256 balance;
    }
    // 声明事件
    event LOG_SWAP(
    ...
    // 交换AmountOut的精确值
    // tokenAmountIn spotPriceBefore tokenAmountIn spotPriceAter 
