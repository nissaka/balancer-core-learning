...
import { getLpListByTokenMintAddresses, getPoolListByTokenMintAddresses, isOfficalMarket, } from '@/utils/pools';
...
let res = {
    ...
    userCheckUnofficialMint: undefined,
    ...
    userNeedAmmIdOrMarket: undefined,
    ...
    onCoinSelect(tokenInfo) {
        var _a, _b;
        if (tokenInfo !== null) {
            if (this.selectFromCoin) {
                this.fromCoin = cloneDeep(tokenInfo);
                if (((_a = this.toCoin) === null || _a === void 0 ? void 0 : _a.mintAddress) === tokenInfo.mintAddress) {
                    this.toCoin = null;
                ...
                if (((_b = this.fromCoin) === null || _b === void 0 ? void 0 : _b.mintAddress) === tokenInfo.mintAddress) {
                    this.fromCoin = null;
                    this.changeCoinAmountPosition();
                }
            }
            ...
            if (this.fromCoin !== null) {
                const newFromCoin = Object.values(TOKENS).find((item) => { var _a; return item.mintAddress === ((_a = this.fromCoin) === null || _a === void 0 ? void 0 : _a.mintAddress); });
                if (newFromCoin === null || newFromCoin === undefined) {
                    this.fromCoin = null;
                }
            }
            if (this.toCoin !== null) {
                const newToCoin = Object.values(TOKENS).find((item) => { var _a; return item.mintAddress === ((_a = this.toCoin) === null || _a === void 0 ? void 0 : _a.mintAddress); });
    ...
    setCoinFromMint(ammIdOrMarket, from, to) {
    ...
    needUserCheckUnofficialShow(ammId) {
        ...
        if (this.fromCoin && this.toCoin && this.liquidity.initialized) {
            ...
            for (const address of Object.keys(this.swap.markets)) {
                if (isOfficalMarket(address)) {
                    ...
                    if ((info.baseMint.toBase58() === fromMint &&
            ...
            if (this.fromCoin.mintAddress && this.toCoin.mintAddress) {
                const liquidityListV4 = getPoolListByTokenMintAddresses(this.fromCoin.mintAddress === TOKENS.WSOL.mintAddress
                    ? NATIVE_SOL.mintAddress
                    : this.fromCoin.mintAddress, this.toCoin.mintAddress === TOKENS.WSOL.mintAddress
                    ? NATIVE_SOL.mintAddress
                    : this.toCoin.mintAddress, typeof InputAmmIdOrMarket === 'string'
                    ? InputAmmIdOrMarket
                    : undefined);
                const liquidityListV3 = getLpListByTokenMintAddresses(this.fromCoin.mintAddress === TOKENS.WSOL.mintAddress
                    ? NATIVE_SOL.mintAddress
                    : this.fromCoin.mintAddress, this.toCoin.mintAddress === TOKENS.WSOL.mintAddress
                    ? NATIVE_SOL.mintAddress
                    : this.toCoin.mintAddress, typeof InputAmmIdOrMarket === 'string'
                ...
                if (liquidityListV4.length === 1 &&
                    ...
                    setTimeout(() => {
                        ...
                        (item) => liquidityListV4.find((liquidityItem) => liquidityItem.ammId === item.ammId));
                    }, 1);
                    return;
                }
                this.lpMintAddress = lpMintAddress !== null && lpMintAddress !== void 0 ? lpMintAddress : '';
                this.initialized = true;
                this.ammId = ammId;
    ...
    updateAmounts() {
        ...
        if (this.fromCoin && this.toCoin && this.ammId && this.fromCoinAmount) {
            // amm
            const poolInfo = Object.values(this.$accessor.liquidity.infos).find((p) => p.ammId === this.ammId);
            const { amountOut, amountOutWithSlippage, priceImpact, } = getSwapOutAmount(poolInfo, this.fromCoin.mintAddress, this.toCoin.mintAddress, this.fromCoinAmount, this.setting.slippage);
            if (!amountOut.isNullOrZero()) {
                ...
                price = +new TokenAmount(parseFloat(toCoinAmount) / parseFloat(this.fromCoinAmount), this.toCoin.decimals, false).fixed();
                impact = priceImpact;
        ...
        if (this.fromCoin &&
            ...
            // serum
            const { amountOut, amountOutWithSlippage, priceImpact } = getOutAmount(this.market, this.asks, this.bids, this.fromCoin.mintAddress, this.toCoin.mintAddress, this.fromCoinAmount, this.setting.slippage);
    ...
    placeOrder() {
        ...
        if (this.isWrap) {
            ...
            // @ts-ignore
            get(this.wallet.tokenAccounts, `${this.toCoin.mintAddress}.tokenAccountAddress`), this.fromCoinAmount)
                .then((txid) => {
                var _a, _b;
                ...
                //       ]),
                //   })
                const description = `Unwrap ${this.fromCoinAmount} ${(_a = this.fromCoin) === null || _a === void 0 ? void 0 : _a.symbol} to ${this.toCoinAmount} ${(_b = this.toCoin) === null || _b === void 0 ? void 0 : _b.symbol}`;
            ...
            get(this.wallet.tokenAccounts, `${this.fromCoin.mintAddress}.tokenAccountAddress`), 
            // @ts-ignore
            get(this.wallet.tokenAccounts, `${this.toCoin.mintAddress}.tokenAccountAddress`), this.fromCoinAmount, this.toCoinWithSlippage)
                .then((txid) => {
                var _a, _b;
                console.log('Transaction has been sent', txid);
                //   this.$notify.info({
                ...
                //       ]),
                //   })
                const description = `Swap ${this.fromCoinAmount} ${(_a = this.fromCoin) === null || _a === void 0 ? void 0 : _a.symbol} to ${this.toCoinAmount} ${(_b = this.toCoin) === null || _b === void 0 ? void 0 : _b.symbol}`;
                //   this.$accessor.transaction.sub({ txid, description })
            })
                .catch((error) => {
                console.log('error', 'swap failed');
            ...
            place(this.$web3, 
            // @ts-ignore
            this.$wallet, this.market, this.asks, this.bids, 
            // @ts-ignore
            this.fromCoin.mintAddress, 
            // @ts-ignore
            this.toCoin.mintAddress, 
            ...
            get(this.wallet.tokenAccounts, `${this.fromCoin.mintAddress}.tokenAccountAddress`), 
            // @ts-ignore
            get(this.wallet.tokenAccounts, `${this.toCoin.mintAddress}.tokenAccountAddress`), this.fromCoinAmount, this.setting.slippage)
                .then((txid) => {
                var _a, _b;
                console.log('Transaction has been sent', txid);
                //   this.$notify.info({
    ...
    async fetchUnsettledByMarket() {
        var _a, _b;
        if (this.isFetchingUnsettled)
            return;
        if (!this.$web3 || !this.$wallet || !this.market)
            return;
        ...
        try {
            ...
            this.baseSymbol = (_a = info.baseSymbol) !== null && _a !== void 0 ? _a : '';
            this.baseUnsettledAmount = info.baseUnsettledAmount;
            this.quoteSymbol = (_b = info.quoteSymbol) !== null && _b !== void 0 ? _b : '';
    ...
    settleFunds(from) {
        ...
        //   duration: 0,
        // })
        let baseMint = this.market.baseMintAddress.toBase58();
        let quoteMint = this.market.quoteMintAddress.toBase58();
        ...
        settleFund(this.$web3, this.market, this.unsettledOpenOrders, this.$wallet, baseMint, quoteMint, baseWallet, quoteWallet)
            .then((txid) => {
            console.log('Transaction has been sent', txid);
            // this.$notify.info({
            //   key,
