...
export function getTokenSymbolByMint(mint) {
...
export const TOKENS = {
    ...
    LIKE: {
        ...
        details: 'Only1 is the first NFT-powered social platform built on the Solana blockchain. Mixing social media, an NFT marketplace, a scalable blockchain, and the native token — $LIKE, Only1 offers fans a unique way of connecting with the creators they love.\n\nBy using the Only1 platform, fans will have the ability to invest, access, and earn from the limited edition contents created by the world’s largest influencers/celebrities, all powered by NFTs.',
        docs: {
...
function addUserLocalCoinMint() {
    ...
    const localMintList = (localMintStr !== null && localMintStr !== void 0 ? localMintStr : '').split('---');
...
function addTokensSolanaFunc(tokens) {
    tokens.forEach((itemToken) => {
    ...
    if (window.localStorage.addSolanaCoin) {
        window.localStorage.addSolanaCoin.split('---').forEach((itemMint) => {
...
function updateTokenTagsChange() {
    var _a;
    const userSelectSource = (_a = window.localStorage.userSelectSource) !== null && _a !== void 0 ? _a : '';
    const userSelectSourceList = userSelectSource.split('---');
