...
import { canWrap, getLpMintByTokenMintAddresses, getPoolByLpMintAddress, getPoolByTokenMintAddresses, LIQUIDITY_POOLS } from '@/utils/pools';
import { TokenAmount } from '@/utils/safe-math';
import { LP_TOKENS, NATIVE_SOL, TOKENS } from '@/utils/tokens';
import { commitment, createAssociatedTokenAccountIfNotExist, createTokenAccountIfNotExist, getMultipleAccounts, sendTransaction } from '@/utils/web3';
...
export function getPrice(poolInfo, coinBase = true) {
...
export function getOutAmount(poolInfo, amount, fromCoinMint, toCoinMint, slippage) {
    const { coin, pc } = poolInfo;
...
/* eslint-disable */
export async function addLiquidity(connection, wallet, poolInfo, fromCoinAccount, toCoinAccount, lpAccount, fromCoin, toCoin, fromAmount, toAmount, fixedCoin) {
    ...
    if (poolInfo.coin.mintAddress === NATIVE_SOL.mintAddress) {
        wrappedCoinSolAccount = await createTokenAccountIfNotExist(connection, wrappedCoinSolAccount, owner, TOKENS.WSOL.mintAddress, coinAmount + 1e7, transaction, signers);
    }
    let wrappedSolAccount;
    if (poolInfo.pc.mintAddress === NATIVE_SOL.mintAddress) {
        wrappedSolAccount = await createTokenAccountIfNotExist(connection, wrappedSolAccount, owner, TOKENS.WSOL.mintAddress, pcAmount + 1e7, transaction, signers);
    }
    let userLpTokenAccount = await createAssociatedTokenAccountIfNotExist(lpAccount, owner, poolInfo.lp.mintAddress, transaction);
    transaction.add(poolInfo.version === 4
        ? addLiquidityInstructionV4(new PublicKey(poolInfo.programId), new PublicKey(poolInfo.ammId), new PublicKey(poolInfo.ammAuthority), new PublicKey(poolInfo.ammOpenOrders), new PublicKey(poolInfo.ammTargetOrders), new PublicKey(poolInfo.lp.mintAddress), new PublicKey(poolInfo.poolCoinTokenAccount), new PublicKey(poolInfo.poolPcTokenAccount), new PublicKey(poolInfo.serumMarket), wrappedCoinSolAccount ? wrappedCoinSolAccount : userCoinTokenAccount, wrappedSolAccount ? wrappedSolAccount : userPcTokenAccount, userLpTokenAccount, owner, coinAmount, pcAmount, fixedCoin === poolInfo.coin.mintAddress ? 0 : 1)
        : addLiquidityInstruction(new PublicKey(poolInfo.programId), new PublicKey(poolInfo.ammId), new PublicKey(poolInfo.ammAuthority), new PublicKey(poolInfo.ammOpenOrders), new PublicKey(poolInfo.ammQuantities), new PublicKey(poolInfo.lp.mintAddress), new PublicKey(poolInfo.poolCoinTokenAccount), new PublicKey(poolInfo.poolPcTokenAccount), new PublicKey(poolInfo.serumMarket), wrappedCoinSolAccount ? wrappedCoinSolAccount : userCoinTokenAccount, wrappedSolAccount ? wrappedSolAccount : userPcTokenAccount, userLpTokenAccount, owner, coinAmount, pcAmount, fixedCoin === poolInfo.coin.mintAddress ? 0 : 1));
...
export async function removeLiquidity(connection, wallet, poolInfo, lpAccount, fromCoinAccount, toCoinAccount, amount) {
    if (!connection || !wallet)
        throw new Error('Miss connection');
    if (!poolInfo)
        throw new Error('Miss pool infomations');
    ...
    if (poolInfo.coin.mintAddress === NATIVE_SOL.mintAddress) {
        newFromTokenAccount = await createTokenAccountIfNotExist(connection, newFromTokenAccount, owner, TOKENS.WSOL.mintAddress, null, transaction, signers);
        needCloseFromTokenAccount = true;
    }
    else {
        newFromTokenAccount = await createAssociatedTokenAccountIfNotExist(fromCoinAccount, owner, poolInfo.coin.mintAddress, transaction);
    }
    let needCloseToTokenAccount = false;
    let newToTokenAccount;
    if (poolInfo.pc.mintAddress === NATIVE_SOL.mintAddress) {
        newToTokenAccount = await createTokenAccountIfNotExist(connection, newToTokenAccount, owner, TOKENS.WSOL.mintAddress, null, transaction, signers);
        needCloseToTokenAccount = true;
    }
    else {
        newToTokenAccount = await createAssociatedTokenAccountIfNotExist(toCoinAccount, owner, poolInfo.pc.mintAddress === NATIVE_SOL.mintAddress ? TOKENS.WSOL.mintAddress : poolInfo.pc.mintAddress, transaction);
    }
    transaction.add(poolInfo.version === 4
        ? removeLiquidityInstructionV4(new PublicKey(poolInfo.programId), new PublicKey(poolInfo.ammId), new PublicKey(poolInfo.ammAuthority), new PublicKey(poolInfo.ammOpenOrders), new PublicKey(poolInfo.ammTargetOrders), new PublicKey(poolInfo.lp.mintAddress), new PublicKey(poolInfo.poolCoinTokenAccount), new PublicKey(poolInfo.poolPcTokenAccount), new PublicKey(poolInfo.poolWithdrawQueue), new PublicKey(poolInfo.poolTempLpTokenAccount), new PublicKey(poolInfo.serumProgramId), new PublicKey(poolInfo.serumMarket), new PublicKey(poolInfo.serumCoinVaultAccount), new PublicKey(poolInfo.serumPcVaultAccount), new PublicKey(poolInfo.serumVaultSigner), new PublicKey(lpAccount), newFromTokenAccount, newToTokenAccount, owner, lpAmount)
        : removeLiquidityInstruction(new PublicKey(poolInfo.programId), new PublicKey(poolInfo.ammId), new PublicKey(poolInfo.ammAuthority), new PublicKey(poolInfo.ammOpenOrders), new PublicKey(poolInfo.ammQuantities), new PublicKey(poolInfo.lp.mintAddress), new PublicKey(poolInfo.poolCoinTokenAccount), new PublicKey(poolInfo.poolPcTokenAccount), new PublicKey(poolInfo.poolWithdrawQueue), new PublicKey(poolInfo.poolTempLpTokenAccount), new PublicKey(poolInfo.serumProgramId), new PublicKey(poolInfo.serumMarket), new PublicKey(poolInfo.serumCoinVaultAccount), new PublicKey(poolInfo.serumPcVaultAccount), new PublicKey(poolInfo.serumVaultSigner), new PublicKey(lpAccount), newFromTokenAccount, newToTokenAccount, owner, lpAmount));
...
export function addLiquidityInstruction(programId, 
...
export function addLiquidityInstructionV4(programId, 
// tokenProgramId: PublicKey,
// amm
...
export function removeLiquidityInstruction(programId, 
// tokenProgramId: PublicKey,
// amm
...
export function removeLiquidityInstructionV4(programId, 
// tokenProgramId: PublicKey,
// amm
...
export async function getLpMintInfo(conn, mintAddress, coin, pc) {
    var _a, _b;
...
export function getLiquidityInfoSimilar(ammIdOrMarket, from, to) {
